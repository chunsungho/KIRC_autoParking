...
#vidcap = cv2.VideoCapture('carNumber.mp4')
    ...
    if ret:
        ...
        if(frame_skip % 5 == 0):
            ...
            for i, plate_img in enumerate(plate_imgs):
                ...
                chars_eng = pytesseract.image_to_string(img_result, lang='eng', config='--psm 7 --oem 0')
                result_chars_eng = ''
                has_digit = False
                for c in chars_eng:
                ...
                nlen_str = len(result_chars_eng)
                sCar_number = result_chars_eng
                if nlen_str >= 4:
                    sCar_number = sCar_number[nlen_str - 4:nlen_str]
                    if pre_car_number == sCar_number:
                        nCnt += 1
                        if nCnt >= 5:
                            print('정답:' + sCar_number)

                            if has_digit and len(result_chars_eng) > longest_text:
                                longest_idx = i
